...
<div id="wrapperlogin" >
<center>
<form role="form" class="form-inline" action="searchnotices.php" method="POST">
<h1>Search</h1>
		...
		$query="SELECT `id`,`date`,`title`,`frm`,`message` FROM `notices` WHERE `title` LIKE '%".$ename."%' ORDER BY `id` DESC";
			if($q=mysql_query($query)){
			 $count =mysql_num_rows($q);
						...
						echo '<tr><td>'.$id.'</td><td>'.$date.'</td><td>'.$title.'</td><td>'.$frm.'</td><td>'.$message.'</td><td><a href="adminupdatenotices.php?id='.$id.'">Edit</a></td><td><a href="admindeletenotices.php?id='.$id.'">Delete</a></td></tr>';
							}
				} 
